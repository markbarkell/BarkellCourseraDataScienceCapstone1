...
buildmapping <- function() {
  for(filename in c(enUsBlogsPath, enUsNewsPath, enUsTwitterPath)) {
    ...
    for(line in readLines(filename)) {
      ...
      preparedLine2 <- gsub("(i|you|we|they) (would|d|a?m|ll|will|have|ve|shall|be|can|is) ", "sptrv ", preparedLine, perl = TRUE)
      preparedLine2 <- gsub("(i|you|we|they) (wouldn't|would not|a?m|will not|haven't|have not|shall not|cannot|can not|isn't|is not) ", "sptrvn ", preparedLine2, perl = TRUE) 
